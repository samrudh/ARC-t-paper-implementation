# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 18:35:36 2019
...
def learnAymmTransform(XA, yA, XB, yB):
#    debug
#    XA = xA
#    XB = xB
...
for j in range(NUM_RUNS):
    ...
    for g in range(len(gamma_set)):
        ...
        gamma=gamma_set[g]
#        xA=XA[np.uint8(trexsA),:]
#        yA=YA[np.uint8(trexsA)];
#        xB=XB[np.uint8(trexsB),:]; 
#        yB=YB[np.uint8(trexsB)];
       # print(learnAymmTransform(xA, yA, xB, yB))
        print(learnAymmTransform(XA[np.uint8(trexsA),:],yA[np.uint8(trexsA)],XB[np.uint8(trexsB),:], yB[np.uint8(trexsB)]))
